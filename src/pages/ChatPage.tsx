...
import type { ChatMessage, ChatMode, ChatStatus, IncomingFriendRequest, VideoSignal } from "../lib/use-chat";
...
interface ChatPageProps {
  ...
  incomingFriendRequest: IncomingFriendRequest | null;
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col relative overflow-hidden">
            ...
            {isMatched && (
              ...
              <button
                onClick={() => {
                  setFriendStatus("");
                  onSendFriendRequest()
                    .then(() => setFriendStatus("Request sent"))
                    .catch((error) => setFriendStatus(error instanceof Error ? error.message : "Login required"));
                }}
                className="p-2 text-gray-500 hover:text-emerald-400 hover:bg-emerald-400/10 rounded-lg transition-all"
                title={currentUser ? "Send friend request" : "Login to send friend requests"}
              >
                <UserPlus size={18} />
              </button>
              <button
      ...
      <main className="flex-1 overflow-y-auto relative z-10">
        <div className="max-w-3xl mx-auto px-4 py-6 space-y-3">
          ...
          {isMatched && friendStatus && (
            <div className="mx-auto max-w-md rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-center text-sm text-emerald-100">
              {friendStatus}
            </div>
          )}

          {isMatched && incomingFriendRequest && (
            <div className="mx-auto max-w-md rounded-xl border border-cyan-400/20 bg-cyan-400/10 p-4 text-sm text-cyan-50">
              <div className="font-semibold">{incomingFriendRequest.senderUsername} sent a friend request</div>
              <div className="mt-3 flex gap-2">
                <button
                  type="button"
                  onClick={() =>
                    onRespondFriendRequest(incomingFriendRequest.id, "accept")
                      .then(() => setFriendStatus("Friend added"))
                      .catch((error) => setFriendStatus(error instanceof Error ? error.message : "Login required"))
                  }
                ...
                <button
                  type="button"
                  onClick={() => onRespondFriendRequest(incomingFriendRequest.id, "reject")}
                  className="rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-gray-200"
                >
                  Reject
                </button>
              </div>
